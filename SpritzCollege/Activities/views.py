...
def string_for_title (string, status):
    out = "searching for " 
    if string: 
        out += string
        if status: 
    ...
    def get_queryset(self):
        ...
        if search_string and search_where:
            ...
            if search_where == "Name":
                filtered_queryset = full_queryset.filter(name__icontains=search_string, status=status)
            elif search_where == "Description":
                filtered_queryset = full_queryset.filter(description__icontains=search_string, status=status)
        ...
        if not filtered_queryset.exists() and search_performed:
            messages.add_message(self.request, messages.INFO, 'No results found for your search criteria.')
...
def course_pdf(request, course_id):
    ...
    if course.image and hasattr(course.image, 'path'):
        try:
            p.drawImage(course.image.path, 100, image_y_position, width=4*inch, height=image_height, preserveAspectRatio=True, mask='auto')
    ...
    if user:
        ...
        p.drawString(100, text_start_position - 20, f"Requested by: {user.username} on {timezone.localtime().strftime('%Y-%m-%d %H:%M')}")

    p.setFont("Helvetica", 12)
    p.drawString(100, text_start_position - 40, f"Duration: {course.start_date} to {course.end_date}")
    p.drawString(100, text_start_position - 60, f"Category: {course.get_category_display()}")
...
class SubscriptionListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = f"{self.request.user.username}  \u2192  Courses Panel"
        return context
